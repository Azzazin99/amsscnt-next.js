...
async function main() {
  console.log("Seeding budget_money_return from AMSS.sql...");
  ...
  for await (const line of rl) {
    ...
    if (trimmed.startsWith("INSERT INTO `budget_money_return`")) {
      insideStmt = true;
      currentStmt = trimmed;
      if (trimmed.endsWith(";")) {
        try {
  ...
  console.log(`Finished seeding budget_money_return! Total insert blocks: ${count}`);
}
