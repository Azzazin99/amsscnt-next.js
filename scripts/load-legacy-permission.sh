#!/usr/bin/env bash
# Extract legacy permission_* tables from AMSS_reduced.sql and load them into MySQL.
# Usage: bash scripts/load-legacy-permission.sh
...
TABLES=(
  ...
  permission_person_set
)

TMPFILE=$(mktemp)
trap "rm -f $TMPFILE" EXIT

echo "Extracting legacy permission tables from $DUMP..."
...
echo "Done. Legacy permission tables loaded."
echo "Now run: npx tsx scripts/import/import-permission-standalone.ts"
