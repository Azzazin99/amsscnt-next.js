...
type Ctx = RouteContext<"/api/bookregister/send/[id]/files/[fileId]">;

async function requireContext(sendId: number) {
...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const sendId = Number(idParam);
  const fileId = Number(fileIdParam);
  if (!Number.isFinite(sendId) || !Number.isFinite(fileId)) {
    return NextResponse.json({ message: "Invalid" }, { status: 400 });
  }

  const required = await requireContext(sendId);
  if (!required.ok) {
  ...
  const [fileRow] = await db
    ...
    .where(
      and(
        eq(registerSendFiles.id, fileId),
        eq(registerSendFiles.refId, required.row.refId),
      ),
    )
    .limit(1);
  ...
  const fullPath = resolveSendFilePath(safeName);
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  await db.delete(registerSendFiles).where(eq(registerSendFiles.id, fileRow.id));

  try {
    await deleteSendFileFromStorage(fileRow.fileName);
  } catch {
