...
export default async function LaPermissionsPage() {
  ...
  const rows = await listLeaveStaffPermissions();
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link
          href="/modules/leave/permissions/new"
        ...
        <table className="w-full min-w-[560px] text-sm">
          <thead>
            <tr className="border-b bg-muted/50 text-left">
              ...
              <th className="px-3 py-3 text-center font-medium">p2 บันทึก</th>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row, index) => (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <LeavePermissionDeleteButton id={row.id} />
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    <Link
                      href={`/modules/leave/permissions/${row.id}/edit`}
