...
export default async function BudgetStatusChangeBudgetPage({ searchParams }: Props) {
  const { user, perms } = await requireBudgetAccess();
  const canWrite = canChangeBudgetStatus(user, perms, "budget");
  ...
  return (
    <BudgetMainKindList
      title={`ปรับปรุง/เปลี่ยนแปลงงบประมาณ ${activeYear.budgetYear}`}
      basePath="/modules/budget/status-change/budget"
      mode="change"
      rows={rows}
      total={total}
      page={page}
      ...
      deleteAction={
        canWrite ? deleteBudgetStatusChange.bind(null, "budget") : undefined
      }
    />
  );
}
