...
export default async function BudgetStatusChangeIncomePage({ searchParams }: Props) {
  const { user, perms } = await requireBudgetAccess();
  const canWrite = canChangeBudgetStatus(user, perms, "income");
  ...
  return (
    <BudgetMainKindList
      title={`ปรับปรุงเงินรายได้แผ่นดิน ${activeYear.budgetYear}`}
      basePath="/modules/budget/status-change/income"
      mode="change"
      rows={rows}
      total={total}
      page={page}
      ...
      deleteAction={
        canWrite ? deleteBudgetStatusChange.bind(null, "income") : undefined
      }
    />
  );
}
