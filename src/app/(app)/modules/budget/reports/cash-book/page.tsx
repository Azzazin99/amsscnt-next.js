...
type Props = {
  searchParams: Promise<{
    year?: string;
    type_id_index?: string;
    page?: string;
    cal_id?: string;
  }>;
};

export default async function BudgetReportCashBookPage({
  searchParams,
}: Props) {
  ...
  const typeIndex = params.type_id_index || "";
      ...
      <div className="text-center space-y-1">
        ...
        <form className="flex flex-wrap items-center justify-end gap-2 pt-2">
          ...
          <select
            ...
            className="h-9 px-3 rounded-md border border-input bg-background font-medium text-foreground text-xs sm:text-sm focus:outline-none"
          >
            <option value="">ทุกประเภท</option>
            <option value="2">เงินงบประมาณ</option>
            <option value="1">เงินนอกงบประมาณ</option>
            <option value="3">เงินรายได้แผ่นดิน</option>
          </select>
        </form>
      </div>

      {/* Main Cash Book Table View (13 columns matching screenshot) */}
          ...
          <tbody className="divide-y divide-border/30">
            {report.rows.length === 0 ? (
              <tr>
                ...
                >
                  ไม่พบข้อมูลรายการในสมุดเงินสดในปีงบประมาณ {selectedYear}
                </td>
              </tr>
            ) : (
              report.rows.map((row) => (
                ...
                >
                  ...
                  <td className="px-2 py-1.5 text-center">
                    <Link
                      href={`/modules/budget/reports/cash-book?cal_id=${row.id}&page=${page}&type_id_index=${typeIndex}`}
