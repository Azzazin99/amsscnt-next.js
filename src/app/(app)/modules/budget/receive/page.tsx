...
import { buildBudgetReceiveUrl } from "@/lib/budget/list-url";
import {
  PAGE_SIZE,
  countBudgetMain,
  getActiveBudgetYear,
...
export default async function BudgetReceivePage({ searchParams }: Props) {
    ...
    <section className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-primary">
          ทะเบียนรับเงินงบประมาณ {activeYear.budgetYear}
        </h2>
      ...
      <BudgetListFilters q={parsed.q} basePath="/modules/budget/receive" />
        ...
        <table className="w-full min-w-[880px] text-sm">
          <thead>
            <tr className="border-b bg-muted/50 text-left">
              ...
              <th className="px-3 py-3 font-medium">ลักษณะ</th>
              ...
              rows.map((row, i) => (
                <tr key={row.id} className={i % 2 === 0 ? "bg-card" : "bg-muted/20"}>
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <Link href={`/modules/budget/receive/${row.id}`} className="text-primary hover:underline">
                      ดู
                    </Link>
                  </td>
                </tr>
      ...
      <ListPagination
        ...
        hrefForPage={(p) => buildBudgetReceiveUrl({ page: p, q: parsed.q })}
      />
    </section>
  );
}
