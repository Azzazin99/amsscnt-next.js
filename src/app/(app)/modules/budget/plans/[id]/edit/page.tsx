...
export default async function BudgetPlanEditPage({ params }: Props) {
  ...
  const row = await getBudgetCodeItem("plans", id);
  if (!row) notFound();

  return (
    <section className="space-y-4">
      <h2 className="text-lg font-semibold text-primary">แก้ไขแผนงาน</h2>
      <BudgetCodeForm
        action={updateBudgetCodeItem.bind(null, "plans", id)}
