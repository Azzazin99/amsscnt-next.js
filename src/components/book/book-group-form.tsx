...
export function BookGroupForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (
        ...
        router.push(`/modules/book/groups/${result.id}/edit`);
        router.refresh();
        return;
      }
      router.refresh();
  ...
  async function handleDelete() {
    ...
    if (!window.confirm("ลบกลุ่มหนังสือนี้?")) return;
    setDeleting(true);
    try {
      ...
      router.push("/modules/book/groups");
      router.refresh();
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-4">
      ...
      <div className="space-y-2 rounded-lg border p-4">
        <span className="text-sm font-medium">สมาชิก (โรงเรียน)</span>
          ...
          {schools.map((s) => (
            ...
            >
              <input
                ...
                name="schoolIds"
                value={s.id}
                defaultChecked={defaultValues?.schoolIds?.includes(s.id)}
              />
              <span>
                {s.schoolCode} {s.name}
              </span>
