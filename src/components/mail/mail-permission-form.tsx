...
import type { StaffOption } from "@/lib/mail/queries";

type MailPermissionFormProps = {
...
export function MailPermissionForm({
  action,
  staffOptions,
  title,
  cancelHref,
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-lg space-y-4">
      ...
      <fieldset className="space-y-2">
        <legend className="text-sm font-medium">อนุญาตให้เป็นเจ้าหน้าที่ (p1)</legend>
        <div className="flex gap-6">
          ...
          <label className="flex cursor-pointer items-center gap-2 text-sm">
            <input
              ...
              name="p1"
              value="false"
              defaultChecked={defaultValues?.p1 === false}
            />
            ไม่ใช่
          </label>
        </div>
