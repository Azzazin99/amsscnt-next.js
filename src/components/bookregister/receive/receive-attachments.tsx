...
export function ReceiveAttachments({ receiveId, className }: Props) {
  const [files, setFiles] = useState<ReceiveFileRow[]>([]);
  ...
  const baseUrl = useMemo(
    () => `/api/bookregister/receive/${receiveId}/files`,
    [receiveId],
  );
  ...
  async function refresh() {
    ...
    try {
      ...
      const data = (await res.json()) as ReceiveFileRow[] | { message?: string };
      if (!res.ok) {
        throw new Error(
  ...
  return (
    <section className={cn("mt-6 rounded-xl border bg-card p-4", className)}>
      ...
      <div className="mt-3 space-y-2">
        <label htmlFor="fileDes" className="text-sm font-medium">
