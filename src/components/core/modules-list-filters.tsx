...
export function ModulesListFilters({ q, status }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 rounded-xl border bg-card p-4 shadow-sm sm:flex-row sm:flex-wrap sm:items-end">
      <div className="min-w-[200px] flex-1 space-y-1.5">
        <label htmlFor="modules-q" className="text-sm font-medium">ค้นหา</label>
        <input id="modules-q" name="q" type="search" defaultValue={q} placeholder="ชื่อหรือ slug" className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm" />
      </div>
      <div className="space-y-1.5 sm:w-40">
        <label htmlFor="modules-status" className="text-sm font-medium">สถานะ</label>
        <select id="modules-status" name="status" defaultValue={status} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm">
